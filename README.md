...
RISC-V (RV32IMC) 命令セットをターゲットとした、高速かつ軽量なエミュレータの実装です。
1990 年代のゲーム機を模したファンタジーコンソールの基盤として開発されています。

## 特徴

- **サポート命令セット**: RV32I, RV32M, RV32C, Zicsr
- **特権モード**: Machine Mode (M-Mode), User Mode (U-Mode)
- **周辺デバイス**:
  - **CLINT**: タイマー割り込みおよびソフトウェア割り込みをサポート
  - **PLIC**: 外部割り込みの優先度管理、Claim/Complete 機構をサポート
- **高速実行**: ページベースのインストラクションキャッシュを搭載
...
- [CSR 命令の実装仕様](docs/csr.md)
- [トラップ処理 (Trap Handling)](docs/exception.md)
- [CLINT 実装方針](docs/clint.md)
- [PLIC 実装方針](docs/plic.md)
...
### 全テストの一括実行
ディレクトリを指定することで、その中のすべての `.bin` ファイルをテストとして実行できます。
```bash
cargo run -- tests/bin
```
