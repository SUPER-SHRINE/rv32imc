...
C.OR は CA 形式の命令で、レジスタ `rd'` の値とレジスタ `rs2'` の値とのビット単位の論理和を計算し、結果を `rd'` に格納します。
この命令は、標準の `OR rd, rd, rs2` 命令の圧縮版に相当します。

### 命令フォーマット
```
| 15      10 | 9       7 | 6   5 | 4       2 | 1    0 |
|:----------:|:---------:|:-----:|:---------:|:------:|
| 100011     | rd'/rs1'  | 10    | rs2'      | 01     |
```
- **rd'/rs1'**: 読み込み・書き込みレジスタ (x8-x15)
- **rs2'**: 読み込みレジスタ (x8-x15)
- **op**: `01` (Quadrants 1)
...
C.OR 命令は、第一オペランド `rd'` と第二オペランド `rs2'` とのビット単位の論理和（OR）を計算し、その結果を `rd'` に書き戻します。
オペランド `rd'` と `rs2'` はどちらも 3 ビットで指定され、汎用レジスタ `x8` から `x15`（`s0` から `a7` の一部）のいずれかを選択します。
この命令は、標準の `OR` 命令においてオペランドが特定の範囲のレジスタに収まる場合に、命令長を節約するために使用されます。

## C.AND

C.AND は CA 形式の命令で、レジスタ `rd'` の値とレジスタ `rs2'` の値とのビット単位の論理積を計算し、結果を `rd'` に格納します。
この命令は、標準の `AND rd, rd, rs2` 命令の圧縮版に相当します。

### 命令フォーマット
```
| 15      10 | 9       7 | 6   5 | 4       2 | 1    0 |
|:----------:|:---------:|:-----:|:---------:|:------:|
| 100011     | rd'/rs1'  | 11    | rs2'      | 01     |
```
- **rd'/rs1'**: 読み込み・書き込みレジスタ (x8-x15)
- **rs2'**: 読み込みレジスタ (x8-x15)
- **op**: `01` (Quadrants 1)
...
C.AND 命令は、第一オペランド `rd'` と第二オペランド `rs2'` とのビット単位の論理積（AND）を計算し、その結果を `rd'` に書き戻します。
オペランド `rd'` と `rs2'` はどちらも 3 ビットで指定され、汎用レジスタ `x8` から `x15`（`s0` から `a7` の一部）のいずれかを選択します。
この命令は、標準の `AND` 命令においてオペランドが特定の範囲のレジスタに収まる場合に、命令長を節約するために使用されます。

## C.J

C.J は CJ 形式の命令で、現在の PC に符号付きオフセットを加算してジャンプします。
この命令は、標準の `JAL x0, offset` 命令の圧縮版に相当します。

### 命令フォーマット
```
| 15      13 | 12  12 | 11                    2 | 1    0 |
|:----------:|:------:|:-----------------------:|:------:|
| 101        | imm[11]| imm[4|9:8|10|6|7|3:1|5] | 01     |
```
- **imm**: 12 ビットの符号付き即値（2バイトアラインメント）
  - ビット構成: `imm[11|4|9:8|10|6|7|3:1|5]`
- **op**: `01` (Quadrants 1)
...
C.J 命令は、無条件制御転送（ジャンプ）を行います。
即値 `imm` は 12 ビットの符号付き整数として扱われ、2 バイト単位（下位 1 ビットは常に 0）で PC に加算されます。
これにより、現在の PC から -2048 ～ +2046 バイトの範囲にジャンプできます。
この命令は、リンクレジスタへの書き込みを行わない点を除けば `C.JAL` と同様の動作をします。

## C.BEQZ

C.BEQZ は CB 形式の命令で、レジスタ `rs1'` の値がゼロであれば、現在の PC に符号付きオフセットを加算してジャンプします。
この命令は、標準の `BEQ rs1, x0, offset` 命令の圧縮版に相当します。

### 命令フォーマット
```
| 15      13 | 12  12 | 11      10 | 9        7 | 6                2 | 1    0 |
|:----------:|:------:|:----------:|:----------:|:------------------:|:------:|
| 110        | imm[8] | imm[4:3]   | rs1'       | imm[7:6|2:1|5]     | 01     |
```
- **imm**: 9 ビットの符号付き即値（2バイトアラインメント）
  - ビット構成: `imm[8|4:3|7:6|2:1|5]`
- **rs1'**: 比較対象のレジスタ (x8-x15)
- **op**: `01` (Quadrants 1)
- **funct3**: `110`
...
C.BEQZ 命令は、レジスタ `rs1'` の値をゼロと比較し、等しければ相対ジャンプを行います。
即値 `imm` は 9 ビットの符号付き整数として扱われ、2 バイト単位（下位 1 ビットは常に 0）で PC に加算されます。
これにより、現在の PC から -256 ～ +254 バイトの範囲にジャンプできます。
`rs1'` は 3 ビットで指定され、汎用レジスタ `x8` から `x15`（`s0` から `a7` の一部）のいずれかを選択します。

## C.BNEZ

C.BNEZ は CB 形式の命令で、レジスタ `rs1'` の値がゼロでなければ、現在の PC に符号付きオフセットを加算してジャンプします。
この命令は、標準の `BNE rs1, x0, offset` 命令の圧縮版に相当します。

### 命令フォーマット
```
| 15      13 | 12  12 | 11      10 | 9        7 | 6                2 | 1    0 |
|:----------:|:------:|:----------:|:----------:|:------------------:|:------:|
| 111        | imm[8] | imm[4:3]   | rs1'       | imm[7:6|2:1|5]     | 01     |
```
- **imm**: 9 ビットの符号付き即値（2バイトアラインメント）
  - ビット構成: `imm[8|4:3|7:6|2:1|5]`
- **rs1'**: 比較対象のレジスタ (x8-x15)
...
C.BNEZ 命令は、レジスタ `rs1'` の値をゼロと比較し、等しくなければ相対ジャンプを行います。
即値 `imm` は 9 ビットの符号付き整数として扱われ、2 バイト単位（下位 1 ビットは常に 0）で PC に加算されます。
これにより、現在の PC から -256 ～ +254 バイトの範囲にジャンプできます。
`rs1'` は 3 ビットで指定され、汎用レジスタ `x8` から `x15`（`s0` から `a7` の一部）のいずれかを選択します。
...
C.SLLI は CI 形式の命令で、レジスタ `rd` の値を指定されたビット数だけ左に論理シフトし、結果を `rd` に格納します。
この命令は、標準の `SLLI rd, rd, shamt` 命令の圧縮版に相当します。

### 命令フォーマット
```
| 15      13 | 12    12 | 11      7 | 6        2 | 1    0 |
|:----------:|:--------:|:---------:|:----------:|:------:|
| 000        | shamt[5] | rd/rs1.   | shamt[4:0] | 10     |
```
- **rd/rs1**: 読み込み・書き込みレジスタ (x1-x31)。x0 は指定できません。
- **shamt**: シフト量。RV32C では 1-31 の範囲。
- **op**: `10` (Quadrants 2)
- **funct3**: `000`
...
C.SLLI 命令は、レジスタ `rd` の内容を指定された `shamt` ビット分だけ左に論理シフトし、その結果を `rd` に書き戻します。
空いた下位ビットには 0 が詰められます。
`shamt` は 6 ビット（`shamt[5]` はビット 12、`shamt[4:0]` はビット 6:2）で指定されますが、RV32C では `shamt[5]` は 0 である必要があります。
`rd` に `x0` を指定することはできません。
また、`shamt` が 0 の場合は HINT 命令として予約されています。

## C.LWSP

C.LWSP は CI 形式の命令で、スタックポインタ (x2/sp) からの相対オフセットを用いてメモリから 32 ビット値をロードし、レジスタ `rd` に格納します。

### 命令フォーマット
```
| 15      13 | 12    12 | 11      7 | 6            2 | 1    0 |
|:----------:|:--------:|:---------:|:--------------:|:------:|
| 010        | imm[5]   | rd        | imm[4:2|7:6]   | 10     |
```
- **imm**: 符号なし即値（4バイトアラインメント、最大 252 バイト）
  - ビット構成: `imm[5|4:2|7:6]`
- **rd**: 書き込み先レジスタ (x1-x31)
  - `rd=x0` は予約済みです。
- **op**: `10` (Quadrants 2)
...
```
if rd != 0:
    addr = x[2] + {24'b0, imm[7:2], 2'b0}
    x[rd] = load_word(addr)
```

### 説明

C.LWSP 命令は、スタックポインタ (`sp`) の値に、スケーリングされた符号なし即値を加算して実効アドレスを計算します。
即値は 6 ビットで構成されますが、下位 2 ビットを 0 と見なす（4 バイトアラインメント）ため、実質的には 8 ビットのオフセット（0 から 252 バイト）として機能します。
計算されたアドレスから 4 バイト（ワード）を読み込み、レジスタ `rd` に書き込みます。
`rd` に `x0` を指定することはできません。
この命令は、標準の `LW rd, offset(sp)` 命令の圧縮版に相当します。

## C.JR

C.JR は CR 形式の命令で、レジスタ `rs1` に格納されたアドレスへ無条件ジャンプします。
この命令は、標準の `JALR x0, 0(rs1)` 命令の圧縮版に相当します。
...
| 1000       | rs1       | 00000      | 10     |
```
- **rs1**: ジャンプ先アドレスが格納されているレジスタ (x1-x31)
  - `rs1=x0` は予約済みです。
- **op**: `10` (Quadrants 2)
- **funct4**: `1000`
...
C.JR 命令は、レジスタ `rs1` に格納されたアドレスへジャンプします。
ジャンプ先のアドレスの下位 1 ビットは 0 として扱われます。
`rs1` に `x0` を指定することはできません。
この命令は戻り先アドレスをリンクレジスタに保存しません。

## C.MV

C.MV は CR 形式の命令で、レジスタ `rs2` の値をレジスタ `rd` にコピーします。
この命令は、標準の `ADD rd, x0, rs2` 命令の圧縮版に相当します。
...
- **rd**: 書き込み先レジスタ (x1-x31)
  - `rd=x0` は予約済みです。
- **rs2**: 読み込み元レジスタ (x1-x31)
  - `rs2=x0` は `C.JR` 命令として扱われます。
- **op**: `10` (Quadrants 2)
- **funct4**: `1000`
...
C.MV 命令は、レジスタ `rs2` の値をレジスタ `rd` にコピーします。
`rd` および `rs2` に `x0` を指定することはできません。
`rs2=x0` の場合は `C.JR` 命令として、`rd=x0` かつ `rs2=x0` の場合は予約済みの命令として扱われます。
この命令は、標準の `mv rd, rs2` 疑似命令（実際には `addi rd, rs2, 0` または `add rd, x0, rs2`）と同等です。

## C.EBREAK

C.EBREAK は CR 形式の命令で、デバッガに対して制御を移すために使用されます。
この命令は、標準の `EBREAK` 命令の圧縮版に相当します。
...
C.EBREAK 命令を実行すると、`Breakpoint` 例外（例外コード 3）が発生します。
この命令は、標準の `EBREAK` 命令と全く同じ動作をしますが、16 ビットの命令長でエンコードされます。
主にデバッグ時のブレークポイント設定に使用されます。
このエンコーディング（`1001 00000 00000 10`）は、CR 形式において `rd=x0` かつ `rs2=x0` の場合に相当します。

## C.JALR

C.JALR は CR 形式の命令で、リターンアドレスを `ra` (x1) に保存し、レジスタ `rs1` に格納されたアドレスへ無条件ジャンプします。
この命令は、標準の `JALR ra, 0(rs1)` 命令の圧縮版に相当します。
...
C.JALR 命令は、レジスタ `rs1` に格納されたアドレスへジャンプし、戻り先アドレス（PC + 2）をリンクレジスタ `x1` (`ra`) に書き込みます。
ジャンプ先のアドレスの下位 1 ビットは 0 として扱われます。
`rs1` に `x0` を指定することはできません（`rs1=x0` は `C.EBREAK` 命令に割り当てられています）。

## C.ADD

C.ADD は CR 形式の命令で、レジスタ `rd` の値とレジスタ `rs2` の値を加算し、結果を `rd` に格納します。
この命令は、標準の `ADD rd, rd, rs2` 命令の圧縮版に相当します。
...
| 1001       | rd        | rs2        | 10     |
```
- **rd**: 読み込み・書き込みレジスタ (x1-x31)
  - `rd=x0` は予約済みです。
- **rs2**: 読み込みレジスタ (x1-x31)
  - `rs2=x0` は `C.JALR` または `C.EBREAK` 命令として扱われます。
- **op**: `10` (Quadrants 2)
- **funct4**: `1001`
...
C.ADD 命令は、レジスタ `rd` の値とレジスタ `rs2` の値を加算し、その結果を `rd` に書き戻します。
`rd` および `rs2` に `x0` を指定することはできません。
`rs2=x0` の場合は `C.JALR` または `C.EBREAK` 命令として扱われます。

## C.SWSP

C.SWSP は CSS 形式の命令で、スタックポインタ (x2/sp) からの相対オフセットを用いて、レジスタ `rs2` の 32 ビット値をメモリにストアします。
...
- **imm**: 符号なし即値（4バイトアラインメント、最大 252 バイト）
  - ビット構成: `imm[5:2|7:6]`
- **rs2**: ストアするデータが格納されているレジスタ (x0-x31)
- **op**: `10` (Quadrants 2)
- **funct3**: `110`
...
```
addr = x[2] + {24'b0, imm[7:2], 2'b0}
store_word(addr, x[rs2])
```

### 説明

C.SWSP 命令は、レジスタ `rs2` の 32 ビット値をメモリに書き込みます。
実効アドレスは、スタックポインタ (`sp`) の値に、スケーリングされた符号なし即値を加算して計算されます。
即値は 6 ビットで構成されますが、下位 2 ビットを 0 と見なす（4 バイトアラインメント）ため、実質的には 8 ビットのオフセット（0 から 252 バイト）として機能します。
この命令は、標準の `SW rs2, offset(sp)` 命令の圧縮版に相当します。
