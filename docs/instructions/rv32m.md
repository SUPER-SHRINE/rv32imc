...
REMU (remainder unsigned) は R 形式の命令で、2 つのレジスタの値（32ビット符号なし整数）の除算を行い、剰余（余り）をレジスタに格納します。
...
REMU 命令は、レジスタ `rs1` を被除数、`rs2` を除数として符号なし整数の除算を行い、その剰余を `rd` に格納します。
ゼロ除算の場合、結果は被除数 `rs1` と同じになります。
