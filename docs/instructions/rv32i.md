...
XOR (exclusive-or) は R 形式の命令で、2 つのレジスタの値のビットごとの排他的論理和 (XOR) を計算し、結果をレジスタに格納します。
...
XOR 命令は、レジスタ `rs1` と `rs2` の値のビットごとの排他的論理和を計算し、その結果を `rd` に格納します。

## SRL (Shift Right Logical)

SRL (shift right logical) は R 形式の命令で、レジスタ `rs1` の値をレジスタ `rs2` の下位 5 ビットで指定されたビット数だけ右に論理シフトし、結果をレジスタに格納します。
...
SRL 命令は、レジスタ `rs1` の値を、レジスタ `rs2` の下位 5 ビット（0〜31）で指定されたビット数だけ右に論理シフトします。空いた上位ビットには 0 が詰められます。結果は `rd` に格納されます。

## SRA (Shift Right Arithmetic)

SRA (shift right arithmetic) は R 形式の命令で、レジスタ `rs1` の値をレジスタ `rs2` の下位 5 ビットで指定されたビット数だけ右に算術シフトし、結果をレジスタに格納します。
...
SRA 命令は、レジスタ `rs1` の値を、レジスタ `rs2` の下位 5 ビット（0〜31）で指定されたビット数だけ右に算術シフトします。空いた上位ビットには元の符号ビット（最上位ビット）がコピーされます。結果は `rd` に格納されます。

## OR (OR)

OR (or) は R 形式の命令で、2 つのレジスタの値のビットごとの論理和 (OR) を計算し、結果をレジスタに格納します。
...
OR 命令は、レジスタ `rs1` と `rs2` の値のビットごとの論理和を計算し、その結果を `rd` に格納します。

## AND (AND)

AND (and) は R 形式の命令で、2 つのレジスタの値のビットごとの論理積 (AND) を計算し、結果をレジスタに格納します。
...
AND 命令は、レジスタ `rs1` と `rs2` の値のビットごとの論理積を計算し、その結果を `rd` に格納します。

## FENCE

FENCE 命令は、異なるデバイス（I/O デバイスなど）やスレッド間でのメモリ操作（ロード/ストア）の順序を保証するために使用されます。
...
```
- **fm**: 4ビットのフェンスモード（通常は `0000`）
- **pred**: 先行するメモリ操作のセット（I:命令, R:ロード, W:ストア, O:I/O）
- **succ**: 後続するメモリ操作のセット（I:命令, R:ロード, W:ストア, O:I/O）
- **rs1**: ベースレジスタ（RV32I では 0 に固定）
- **rd**: 書き込み先レジスタ（RV32I では 0 に固定）
- **opcode**: `0001111`
- **funct3**: `000`
...
FENCE 命令は、プログラム内のメモリ操作の実行順序を制御します。`pred`（先行）セットに含まれるすべての操作が、`succ`（後続）セットに含まれるすべての操作よりも前に完了することを保証します。

本エミュレータは現在シングルコア構成であり、ホストマシンのメモリモデルに依存しているため、この命令は実質的に **NOP (No Operation)** として動作します。将来的にマルチコアや高度な I/O をサポートする場合には、適切な同期処理が必要となります。

## FENCE.I

FENCE.I 命令は、メモリへの書き込みと命令フェッチ（読み込み）の同期をとるために使用されます。
...
FENCE.I 命令は、それより前に実行されたストア命令によるメモリへの書き込みが、それ以降に実行される命令フェッチによって正しく読み込まれることを保証します。

自己書き換えコード（Self-modifying code）を実行する場合や、OS が新しいプログラムをメモリにロードした後にそのプログラムを実行する場合に必須となります。命令キャッシュを持つシステムでは、キャッシュのフラッシュなどが行われます。

本エミュレータは現在命令キャッシュをシミュレートしておらず、常にメモリから直接命令をフェッチするため、この命令は実質的に **NOP (No Operation)** として動作します。

## ECALL

ECALL (environment call) 命令は、実行環境（通常はオペレーティングシステム）に対してサービスを要求するために使用されます。
...
```
// 現在の特権モードに応じた例外を発生させる
raise_exception(EnvironmentCallFromUMode | EnvironmentCallFromSMode | EnvironmentCallFromMMode)
```

### 説明

ECALL 命令を実行すると、現在の特権モードに応じて `Environment Call` 例外が発生します。
- User mode から実行された場合: 例外コード 8
- Supervisor mode から実行された場合: 例外コード 9
- Machine mode から実行された場合: 例外コード 11

例外が発生すると、現在の PC は `mepc` に保存され、PC は `mtvec` に設定されたトラップハンドラのアドレスへジャンプします。また、特権モードは Machine モード（または設定により Supervisor モード）に遷移します。

## EBREAK

EBREAK (environment breakpoint) 命令は、デバッガに対して制御を移すために使用されます。
...
```
// Breakpoint 例外を発生させる
raise_exception(Breakpoint)
```

### 説明

EBREAK 命令を実行すると、`Breakpoint` 例外（例外コード 3）が発生します。ECALL と同様に、PC が `mtvec` にジャンプし、デバッグ環境（トラップハンドラ）が呼び出されます。

## MRET

MRET (machine-mode return) 命令は、マシンモードのトラップハンドラから復帰するために使用されます。
...
MRET 命令は、トラップ処理を終了して元の実行フローに戻るために使用されます。
- PC を `mepc` に保存されていたアドレスに戻します。
- `mstatus` レジスタの `MPIE` (Previous Interrupt Enable) を `MIE` (Interrupt Enable) に戻し、`MPIE` を 1 に設定します。
- 特権モードを `MPP` (Previous Privilege) に保存されていた値に戻し、`MPP` を User モード (0) にリセットします。
